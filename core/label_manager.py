...
class LabelManager:
    ...
    def __init__(self, labels_dir: str):
        ...
        self.boxes: List[Tuple[float, float, float, float, float]] = []  # class_id, x_center, y_center, width, height
    
    def clear_boxes(self) -> None:
        """Clear all bounding boxes."""
        self.boxes.clear()
    
    def add_box(self, x_center: float, y_center: float, width: float, height: float, class_id: float = DEFAULT_CLASS_ID) -> None:
        ...
        try:
            with open(label_path, "w") as f:
                for class_id, x_center, y_center, width, height in self.boxes:
                    f.write(f"{class_id} {x_center:.{COORDINATE_PRECISION}f} {y_center:.{COORDINATE_PRECISION}f} "
