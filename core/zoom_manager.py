...
class ZoomManager:
    ...
    def zoom_in_mouse(self) -> bool:
        ...
        if (
            not self._can_zoom_with_debouncing()
            or not self.can_zoom_in_mouse()
            or self.drawing_box
        ):
            return False
    ...
    def _zoom_to_scale(self, new_scale: float, center_on_mouse: bool) -> bool:
        ...
        if center_on_mouse:
            ...
            new_canvas_x, new_canvas_y = self.image_manager.image_to_canvas_coords(
                img_x, img_y
            )
            self.image_manager.image_x = self.last_mouse_x - (
                new_canvas_x - self.image_manager.image_x
            )
            self.image_manager.image_y = self.last_mouse_y - (
                new_canvas_y - self.image_manager.image_y
            )
        else:
